...
const SONG_TARGET_METADATA_CACHE_TTL_MS = 60_000;
...
const PLATFORM_CACHE_TTL_MS = 10_000;
const RESOLVED_BINARY_CACHE_TTL_MS = 5_000;
...
export async function getQueueState(songloft: SongloftCommandApi): Promise<QueuePayload> {
  const resolvedMpc = await resolveBinary(songloft, "mpc");
  if (!resolvedMpc.executableAvailable) {
    return {
      total: 0,
  ...
  const [statusResult, playlistResult, queueMetadata] = await Promise.all([
    execResolvedCommand(songloft, resolvedMpc, ["status"]),
    execResolvedCommand(songloft, resolvedMpc, [
      "--format",
      "%position%||%artist%||%title%||%album%||%time%||%file%",
      "playlist"
    ]),
  ...
  const parsedItems = playlistResult.stdout
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean)
    .map((line) => parseQueuePlaylistLine(line))
...
export async function removeQueueItem(songloft: SongloftCommandApi, position: number) {
  ...
  const playlistResult = await execResolvedCommand(songloft, resolvedMpc, [
    "--format",
    "%file%",
    "playlist"
  ]);
...
async function resolvePlayableTarget(songloft: SongloftCommandApi, song: SongRecord | null | undefined): Promise<string> {
  ...
  if (typeof song.url === "string" && song.url.trim()) {
    const normalizedSongUrl = await buildSongloftAbsoluteUrl(songloft, song.url);
  ...
  if (localPath) {
    const hostedUrl = await buildSongloftHostedFileUrl(songloft, localPath);
    if (hostedUrl) {
      return hostedUrl;
    }
...
export async function playBatch(songloft: SongloftCommandApi, songIds: string[], shuffle: boolean, replaceQueue: boolean = true) {
  if (!songIds || !songIds.length) {
    return;
  }

  const playableTargets: Array<{ target: string; song: SongRecord }> = [];
  for (const songId of songIds) {
    ...
    if (target && song) {
      playableTargets.push({ target, song });
    }
  }
  ...
  let finalTargets = playableTargets;
  if (shuffle) {
    finalTargets = [...playableTargets].sort(() => Math.random() - 0.5);
  }
...
export async function stopManagedMpd(songloft: SongloftCommandApi) {
  ...
  const running = await songloft.command.isRunning(MPD_PROCESS_NAME).catch(() => false);
  try {
    const stoppedByPid = running ? await stopManagedMpdViaPidFile(songloft, runtimeFiles) : false;
    if (running) {
      const stillRunning = await songloft.command.isRunning(MPD_PROCESS_NAME).catch(() => false);
      if (stillRunning) {
        await songloft.command.stop(MPD_PROCESS_NAME);
      } else if (stoppedByPid) {
        songloft.log.info("Managed MPD process terminated via pid file before SDK stop");
      }
    }
    await cleanupManagedRuntimeFiles(songloft);
    await writeActiveSongSnapshot(songloft, null);
  } catch (error) {
    songloft.log.error("Failed to stop managed mpd: " + String(error));
    throw error;
  }
}
